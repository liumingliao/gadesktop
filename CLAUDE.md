...
| 9. B4: CLI feature-complete + background mode + adapter artifact · [playbook](./docs/refactor/B4-cli-bg-artifact.md) · [M1 sub-plan](./docs/refactor/B4-M1-sub-plan.md) | 📋 Playbook ready (升格 2026-05-20) · M1 sub-plan ship (2026-05-20) · M1 impl pending JC review；M2 prereq gate pending (tray spike + dogfood window) | 2-3w · CLI 全部命令（archive / btw / project / llm 等）+ menubar daemon mode（关窗→隐藏）+ Galley Supervisor SOP + galley-supervisor skill + docs/agent-api.md 公开契约 + discovery file 写入。**Playbook 升格 2026-05-20** 400 行：B4-I1..I7 invariants + A1-A14 acceptance + M1-M9 sub-tasks + 9 gotchas + 9 open decisions + migration pattern。**M1 sub-plan 2026-05-20 ship** 661 行：playbook 说 "7 commands" 按 subcommand 实算 11 个 (5 session-write + 4 project + 2 llm) + 4-commit shape (M1.1 prereq GalleyError::RunnerError + helpers → M1.2 session-write → M1.3 project + llm → M1.4 agent-api + tests) + 8 关键决策：session stop=Abort 不 Shutdown (bridge 留活 + 跟 GUI 顶栏对齐) / btw 不持久化 v0.1 transient 保持 / exit code 5=runner_error 引入 (PRD §11.2 已说 agent-api.md 表漏 row M1.1 补) + GalleyError::RunnerError variant / llm list 走 SQLite prefs cache 不 socket (秒级响应 vs 5-10s warmup) / project move = assign_session_to_project 语义 (CLI surface `project move <sid> [--to=<pid>]`，`--to` 缺省 = 拆出 project) / project archive = delete_project (FK CASCADE SET NULL 保 sessions；v0.5 简化，不加 archived 字段 scope creep) / by-name LLM lookup case-insensitive / `--pretty` + `--until=idle` 推 M9 catchall。12 risk + 8 reject + **6 open decisions queued for JC review** (O1 send_message fail 返 partial vs exit 5 / O2 project archive 阻拦 vs SOP 教 / O3 project move 命名 / O4 SOP 演示 / O5 btw origin push event / O6 session kill surface 加否)。**M1 impl 跟 tray spike 完全独立**可并行；M2 仍受 tray spike + dogfood window 双 gate 阻。B4 估的 2-3 周按 B1/B2/B3 节奏可能压缩到 1-2 周 |
