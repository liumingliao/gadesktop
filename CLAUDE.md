...
- **打包脚本**：[`scripts/bundle-python.sh`](scripts/bundle-python.sh)。下载 PBS tarball → 解压到 `desktop/src-tauri/python-bundle/python/` → 删冗余 stdlib（test / tkinter / idlelib / turtledemo / ensurepip / Mac x86 `_tkinter.so` codesign blocker）→ `pip install` 四个 GA core deps 进 bundle 的 `site-packages/`
...
- 测试矩阵：`bridge/tests/` 106/106 在 b063518（新）+ fc6b5ad（旧）两个 baseline 都通过
