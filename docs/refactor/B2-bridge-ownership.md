...
```
Cursor:   T6.1  (M1-M5 done; next: agent-api.md increment for socket protocol)
Status:   🚧 M1-M5 COMPLETE · M6 + M7 pending
Started:  2026-05-19
Last touch: 2026-05-19 — M4+M5 ship same session: CLI session send/watch through
            socket, origin migrations 006/007, 82 tests pass
Predecessor: B1 ✅ (commits 4ee23e3 → 41cdeb5)
Successor:   B3 (useAppStore 拆 slice + 改订阅 Rust event)
Duration:    3 周估计（D16-D30），实际单 session 推到 M5 (~5h cumulative)
```
...
- [ ] **T3.13** Manual smoke (推到 JC dogfood 时跑)：
  ...
  - Windows: 推到 v0.5 ship 前 Windows 机 smoke 时一并
...
- [-] **T5.8** Dogfood v0.2 → v0.5 升级路径校验 — pending JC 手动测（M7 段一并跑）
