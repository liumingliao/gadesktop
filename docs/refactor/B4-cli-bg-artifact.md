...
```
Cursor:   M3 sub-plan + discovery file + Settings → Integration tab (M2 still gated on Windows-machine access for tray spike)
Status:   ✅ M1 COMPLETE · ✅ M4 T4.1 SOP doc shipped · ➡️ M3 next (Mac-implementable, no Windows dependency)
Started:  2026-05-20 (paperwork)
Last touch: 2026-05-20 PM — M4 T4.1 SOP commit shipped (`bf9e607`)：docs/integrations/galley-supervisor-sop.md 434 lines。9 sections following playbook M4 T4.1 spec (你的角色 + Discovery + 命令速查 + 常用 scenario + Destructive 守则 + Origin 约定 + Error handling + Not-in-v0.5 + Self-check + See also)。System-prompt-addendum tone (second person 直接指令)，PRD §11.1 surface 8 个常见 scenario mapped to CLI invocations。中文 v0.5 一版 (T4.5 决策 i18n 推 v0.6+)。Schema 漂移声明：本 SOP 跟 agent-api.md 不一致以 agent-api.md 为准。**M4 T4.2/T4.3 还 gated** — T4.2 SOP 安装到 GA memory/ 是 M3 Settings 按钮的事；T4.3 dogfood 1-2w 是 calendar；T4.4/T4.5 是 iterate + open question。本 commit ship T4.1 标志 SOP 内容版准备好让 M3 install button 有东西嵌入。**Next pickup**: M3 sub-plan + T3.1 discovery file (Rust setup hook) + T3.2 Settings → Integration tab scaffold + T3.5 docs link button——全 Mac-implementable，PATH install (T3.3) + SOP install (T3.4) 可以 same session 或下次推。
