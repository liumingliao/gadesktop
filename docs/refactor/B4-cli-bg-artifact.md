...
- [ ] **T5.4** 写一个示例 scenario — "用 Claude 创建 Galley session 跑某 task" + 截图录屏（v0.5 release notes 用）— **defer to JC dogfood + v0.5 release prep**（screen recording needs JC's screen, can't be done headless; pair with M9 release-notes draft）

### M5 完成标志

- A10 **partial tick** — skill files shipped + Claude Code harness loads it (smoke); full A10 requires JC to fire a real trigger prompt (e.g. "帮我看看 Galley 现在跑啥") and confirm Claude reads discovery file → runs CLI → returns sensible output. Dogfood task carried into v0.5 release-prep window alongside T5.4 screenshot scenario.
...
## M8 · v0.x → v0.5 data migration 真跑 (D64) ✅ COMPLETE (2026-05-20)

最 P0 风险（数据丢失不可恢复）。[B4-I6](#phase-invariants--b4-特有的硬规则) 备份步骤强制。**Re-scope 2026-05-20**：B2 mig 006/007 已 ship 全部 origin 字段，v0.5 无新 schema delta；M8 真活儿 = backup mechanism + 失败处理 + 文档化 rollback。详 [M8 sub-plan §1.1 现状对照](./B4-M8-sub-plan.md#11-现状对照) + [M8 完成 devlog](../devlog/2026-05-20-b4-m8-migration-backup.md)。
...
- [ ] **T8.7** Dogfood migration — **推迟到 v0.6+** prereq（v0.5 没新 mig delta 触发不到 backup 路径，dogfood 无意义；等 v0.5.x / v0.6 第一次加 mig 008+ 时真跑）
...
## M9 · B4 acceptance + v0.5 ship 准备 (D65+) · 📋 Paperwork prep complete (2026-05-20)

v0.5 RC → v0.5 GA 的 release ceremony。Paperwork prep (T9.0 + T9.3 + T9.4 draft + T9.7) shipped 2026-05-20；剩 dogfood + tag + publish 等 calendar/JC gate。
...
- [ ] **T9.1** 跑遍 A1-A14 acceptance — 每条勾掉（M9 ship session 前最后做）。当前 status: A1 ✅ A2 ✅ A3 ⏳ A4 ⏳ A5 ✅ A6 ✅ A7 ✅ (macOS) A8 ✅ A9 partial A10 partial A11 partial ✅ A12 partial A13 ⏳ A14 ⏳
- [ ] **T9.2** Dogfood 一周 — JC 自己用 + 邀请 1-2 个外部 supervisor user 试用（GA 群里挑 active user）。**Calendar gate**
- [x] **T9.3** README 改写 — v0.1 "本地桌面工作台" framing → v0.5 dual-native orchestrator (tagline D `本地 agent team 编排器 —— GUI 给人，CLI 给 supervisor agent`)。功能 today/即将 split 合并 v0.5-present。"给 Supervisor Agent 用 · 集成 v0.5" 新增 section 链接 SOP + Skill。架构图清 🚧 markers。安装 v0.x.x → v0.5.x。`cd desktop` → `cd gui`。Galley CLI 单独构建段加。**DESIGN.md onboarding subtitle 同步推迟 M9 ship 时改 React 组件**
...
- [x] **T9.7** PRD / CLAUDE.md / refactor README 标 v0.5 status — PRD §17 路线图加状态列 + 各阶段时间窗 / 状态 / 描述同步实际进度（v0.1.1 ✅ / B1-B3 ✅ / B4 7/9 + paperwork prep / v0.5 TBD dogfood 后）。docs/refactor/README.md cursor + dashboard B4 row 加 M8 / M9 prep 进度。CLAUDE.md stage 9 row M8 段已在 M8 closeout 加完
- [ ] **T9.8** 写 B4 完成 devlog + v0.5 release devlog（分两个 entry，B4 是工程过程 / v0.5 是产品 milestone）。**Ship 时写**
...
- [ ] v0.5 ship 到 GitHub Latest（T9.5 + T9.6 dogfood 后）
- [x] README align dual-native framing
- [ ] DESIGN.md onboarding subtitle align（M9 ship session）
- [x] PRD §17 路线图 align v0.5 status
- [ ] B4 + v0.5 devlog ship（T9.8 post-ship）
...
- `docs/refactor/B4-M9-sub-plan.md` (NEW, 320+ 行) — scope assessment vs playbook + v0.5 framing decisions (tagline D selected, "Supervisor Agent" + "dual-native" inline explanation pattern) + README rewrite delete/change/keep map + release notes 5-section draft + PRD/refactor README align scope + 4 open decisions
- `README.md` (rewrite) — v0.1 "今天" / "v0.5 之后" 时序分裂整体退役为 v0.5 现在时，v0.1 退化为 blockquote 历史注；功能 today/即将 split 合并为单一 list（含 ⚙️ Galley CLI + 📡 Agent API + 🍱 Background mode + 🔄 backup）；新增 "给 Supervisor Agent 用 · 集成 v0.5" section（GA bot 集成 + Claude Code 集成 + Settings → Integration 一键安装入口）；架构图清 🚧 markers；安装 v0.x.x → v0.5.x；从源码构建 `cd desktop` → `cd gui` + Galley CLI 单独构建段加
- `docs/PRD.md` §17 路线图 — 时间窗列改 "时间窗 / 状态" 列；v0.1 ✅ + v0.1.1 ✅ (2026-05-18) + v0.2 B-refactor 不挡 + Prototype-B3 各 ✅ 日期 + B4 7/9 进行中 + v0.5 TBD ship gated on dogfood
- `docs/refactor/README.md` cursor + dashboard — cursor 改 M8 + M9 prep COMPLETE narrative；B4 row 加 M8 + M9 paperwork prep；新增 "v0.5 milestone" row 标 ship gated；Last-touch 列同 session 多 update OK
...
4. **README "给 Supervisor Agent 用 · 集成 v0.5" 加 dedicated section** — supervisor adapter ecosystem is v0.5's #1 differentiator; links to SOP + Skill + Settings → Integration tab make this discoverable from README without forcing reader into agent-api.md.
5. **README_en.md not updated** — sub-plan O1 resolved with "标 legacy + v0.5 ship 后再做英文重写" to keep session paperwork scope sane. Footer in zh README links `[English README](./README_en.md) (legacy v0.1 — v0.5 rewrite TBD)`.
...
**B4 status after N20**: M1 / M3 / M4 T4.1 / M5 / M6 / M7 / M8 milestones shipped + M9 paperwork prep done. **v0.5 RC acceptance**: A1 ✅ A2 ✅ A3 ⏳ A4 ⏳ A5 ✅ A6 ✅ A7 ✅ A8 ✅ A9 partial A10 partial A11 ✅ partial A12 partial A13 ⏳ A14 ⏳. Ship gate: dogfood + acceptance walkthrough.
...
**Per-decision notes during impl**:
1. **Trigger policy = Strategy A** (字面 honor B4-I6 "schema migration ... hard-coded 备份步骤")。v0.5 不触发 backup (on-disk version == code-side max == 7)；forward-looking 为 v0.6+ 准备。
...
**B4 status after N19**: 7 of 9 milestones shipped (M1 / M3 / M4 T4.1 / M5 / M6 / M7 / M8). Remaining: **M2 menubar daemon** (Windows-machine gated), **M4 T4.2-T4.5** (IM bot calendar-gated), **M9 v0.5 release ceremony** (needs A12 / A13 / A14 full tick + 1-week dogfood). v0.5 RC acceptance: A1 ✅ A2 ✅ A3 ⏳ A4 ⏳ A5 ✅ A6 ✅ A7 ✅ A8 ✅ A9 partial A10 partial A11 ✅ partial A12 partial A13 ⏳ A14 ⏳.

**Open from this session**:
1. T8.7 dogfood real run pushed to v0.6+ prereq (no schema delta in v0.5 to trigger backup path)
...
**Next session pickup options**: (a) **JC dogfood interleave** — M5 trigger validation + M3 button click-through + M7 supervisor annotation smoke test, ~30 min, closes A10 / A12 full ticks + finishes M3 dev-mode validation; cheap interleave; (b) **M8 v0.x → v0.5 data migration** — schema migration 010-014 design + backup strategy + dogfood real run; calendar-gated by JC's accumulated dogfood data; v0.5 ship blocker eventually; (c) **M2 tray spike** — still Windows-machine gated; (d) **M9 sub-plan + release-notes draft** — v0.5 RC checklist work; needs A11 (migration) + A14 (1-week dogfood) to land first. **Recommended**: (a) dogfood interleave — cheap insurance that 3 milestones we shipped today (M5 / M3 / M7) survive live testing before we layer M8 on top. M8 needs JC's actual sessions to be load-bearing for a real migration test, so the order JC dogfood → M8 backup design → M8 implementation feels natural. M2 stays Windows-blocked regardless.
