...
```
Cursor:   T6.1 implementation  (M6 sub-plan ✅ ship 完，等 M5 dogfood + JC review sub-plan，然后 fresh session 推 M6 impl)
Status:   🟢 M2 ✅ · M3 ✅ · M4 ✅ · M5 ✅ (sub-plan + implementation) · M6 ✅ sub-plan (impl 待 fresh session)
Started:  2026-05-19
Last touch: 2026-05-20 — **M6 sub-plan ship**（paperwork-only session, 0 代码改动）。Sub-plan 落地 [`B3-M6-sub-plan.md`](./B3-M6-sub-plan.md) 616 行：audit useAppStore.ts 残留 5 字段 (gaConfig/approvalConfig/yoloMode/yoloIntroSeen/conversationWidth) + 6 action (5 setter + setGAConfig with cross-store fan-out) + hydrateFromDB orchestrator + seedMockSessions forward shim；**关键 decision** single commit M6 + **useAppStore.ts 整文件删除**（不留 composition shim per B3-I6）；hydrateFromDB **拆 prefsStore.hydratePrefs (own field load) + 新建 lib/hydrate.ts (顶层 orchestrator pure module)**；dispatchIPCEvent **删 store param** 改直 import usePrefsStore；5 reverse callers swap (App.tsx 14 hook + Onboarding.tsx 1 hook + runtime.ts gaConfig read + sessions.ts dynamic import + ipc-handlers.ts typeof useAppStore)；R1-R10 risk register + V1-V10 verification + 7 cluster dogfood scenarios + 8 rejected alternatives。M5 dogfood (T5.16) **仍留下次 session 跑** —— M6 impl 启动门 = M5 dogfood 完成 + JC review M6 sub-plan。详 N15。
...
- [x] **A1**: `useAppStore.ts` 拆成 5 个 slice 文件 — ui (66) + runtime (708) + sessions (1054) + messages (584 + rowsToTurns 子文件 123) + useAppStore (465 残留 prefs/hydrate)。messages + ui 守 600 上限；runtime / sessions 超上限 G11 子文件预案标 M6/M7 收尾时机
- [x] **A2**: authoritative state 写入路径：sessions/projects 100% 走 Rust GalleyApi (M4)；runtime spawnBridge / sendIPCCommand 走 Rust RunnerManager (M3b)；messages SQLite 写 (persistUserMessage / persistToolEventApprovalDecision) 在 messagesStore 内 fire-and-forget (B4 才考虑改 trait method 路径)；无直接 setState authoritative 字段的 ad-hoc mutation
- [x] **A3**: display state 仍在 slice 端：ui screen/palette/settings/toasts 纯本地 / sessions activeProjectFilter 纯本地 / runtime pendingLLMIndex 纯本地；无 Rust round-trip indirection
...
- [x] **A9**: TypeScript / Rust 测试全过 (M5 ship 时 126/126 + typecheck 0 + lint 0 + cargo check clean)；性能基线对比留 dogfood
- [x] **A10**: 每个 slice 写 module doc — ui/runtime/sessions/messages 顶部都有 jsdoc 简介所属字段 + 跨 slice 协调 + 历史出处；useAppStore.ts 顶部说明 M5 后只剩 prefs/hydrate
- [ ] **A11**: 老 `useAppStore.ts` 文件清到 < 200 行 — **暂 465 行**，剩 5 字段 + 6 action + hydrate orchestrator + seedMockSessions forward；M6 prefsStore 可清完 (T6.6 推荐删干净)
...
- [ ] **T6.1** 新建 `gui/src/stores/prefs.ts`。fields = `gaConfig` / `approvalConfig` / `yoloMode` / `yoloIntroSeen` / **`conversationWidth`**（playbook 原列 `runtimeInfo` 错 —— M3a 已迁 runtimeStore；正确字段见 sub-plan §1.1）
- [ ] **T6.2** 迁 setGAConfig (+ cross-store fan-out 6 step) / setApprovalRequiredTools / removeAlwaysAllow / setYoloMode (+ broadcast to alive bridges) / acknowledgeYoloIntro / **setConversationWidth**（playbook 漏，sub-plan §1.2 补）
- [ ] ~~**T6.3** Rust 端 emit `prefs-updated` event~~ — sub-plan §1 **拒**（违反 B3-I4 + prefs 无 cross-process consumer），推 B4 之后
- [ ] **T6.4** hydrateFromDB 拆解 → **prefsStore.hydratePrefs (own field load)** + **新建 lib/hydrate.ts (顶层 orchestrator pure module)**。App.tsx mount effect 改 `import { hydrateApp } from "@/lib/hydrate"`
- [ ] ~~**T6.5** 迁 seedMockSessions~~ — 已在 M4b 搬 sessionsStore；M6 内**删 forward shim**（DevTools 用 `__sessions.getState().seedMockSessions()`）
- [ ] **T6.6** `useAppStore.ts` **彻底删除**。同 commit 内为 prefs.ts 加 `__prefs` DevTools exposure 替代 `__store` 入口
- [ ] **T6.7** dispatchIPCEvent **删 store param** + 5 reverse cross-store callers swap (App.tsx / Onboarding.tsx / runtime.ts / sessions.ts / ipc-handlers.ts)
- [ ] **T6.8** TypeScript / lint / cargo check / cargo test 全过
- [ ] **T6.9** Dogfood 1 天 (V1-V10 + 7 cluster scenarios — 详 sub-plan §5 + §6)
- [ ] **T6.10** M6 commit: `Refactor: B3 M6 — extract prefsStore + retire useAppStore.ts`
