...
```
Cursor:   T6.1  (M6 prefsStore — fresh session,先写 M6 sub-plan)
Status:   🟢 M2 ✅ · M3 ✅ · M4 ✅ · M5 ✅ (sub-plan + implementation)
Started:  2026-05-19
Last touch: 2026-05-19 — **M5 完成代码层**。本 session sub-plan + impl 串完（破 sub-plan「M4b dogfood + fresh session」启动门 per JC explicit）。Single commit M5 (`f7fc4e7`) 9 文件 1052/-1160 LOC：useAppStore.ts 1431 → 465 行 (-966 LOC)；新 messages.ts 584 行 + messages/rowsToTurns.ts 123 行 (G11 子文件拆分)；4 处 cross-store transitional stub 全 fix (M3b onClose / M3b LRU / M4b clearSessionRuntime / M4b applyRuntimeUpdate)；activateSession 搬 sessionsStore；deriveSessionStatus 签名换结构化 MessagesView；EMPTY 单例 + Object.freeze + 双 cast。Rust 0 改动 (B3-I4 守) · 126/126 tests · TS typecheck + lint 0 warning。**M5 dogfood (T5.16) 留下次 session** — V5 auto-scroll regression + V6 turnIndexOffset PK correctness + V7 mirror correctness + 7 cluster scenario。M6 启动条件：M5 dogfood 通过 + M6 sub-plan 写作。详 N14。
