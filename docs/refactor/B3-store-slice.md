...
```
Cursor:   T4a.1  (M4a Rust trait + tests, 同 session 一气推完后 fresh session 重开 M4b)
Status:   🟢 M2 ✅ · M3 ✅ · M4 sub-plan ✅ (B3-M4-sub-plan.md 落地，16 trait method 精确列出)
Started:  2026-05-19
Last touch: 2026-05-19 — M4 sub-plan ship。**M4 拆 M4a (Rust trait + tests, 独立 commit) + M4b (frontend sessionsStore 抽离, fresh session)** 两阶段。M4a scope 收敛精确数：12 session method + 4 project method = 16（playbook 「18」是粗估）。setActiveSession / activateSession / setActiveProjectFilter 是纯 display state operation，**不需要 trait method**。R1-R8 risk register + 7 verification gates 列清楚。详 [B3-M4-sub-plan.md](./B3-M4-sub-plan.md)
